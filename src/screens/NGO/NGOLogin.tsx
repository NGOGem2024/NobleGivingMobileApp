...
interface NGOLoginProps{
  navigation:NGOLoginNavigationProp;
}
...
const NGOLogin:React.FC<NGOLoginProps> = ({navigation}) => {
  ...
  // Update your handleLogin function in NGOLogin.tsx
const handleLogin = async (email: string, password: string) => {
    ...
    try {
      const response = await api.post(EndPoint.ngo_login, { email, password, rememberMe });
  
      if (response.data.requiresOtp) {
        // Navigate to OTP verification screen
        navigation.navigate('NgoOTPVerification', { email });
      } else {
